import { Body, Controller, Delete, Get, Param, Post, Put, Query, UseGuards,UploadedFile, UseInterceptors, Request, HttpCode, HttpStatus  } from '@nestjs/common';
import { PaginationDto, PaginationResult } from 'src/commons/paginations';
import { NotificationEntity } from './notification.entity';
import { ApiBearerAuth, ApiBody, ApiTags } from '@nestjs/swagger';
import { DeleteResult, InsertResult, UpdateResult } from 'typeorm';
import AboutDto from './dto/notification.dto';
...
export class NotificationController {
    constructor(private notificationService: NotificationService) {}
     @Get()
    async findAll(@Query()query:PaginationDto):Promise<PaginationResult<NotificationEntity>> {
    ...
    //   console.log(req?.AccessControllerEvent, '===>>>> currentVerifyMode');
      ...
      if(req&&req.AccessControllerEvent&&req?.AccessControllerEvent?.currentVerifyMode !=='invalid'){
        console.log('===>>>> items in',req.AccessControllerEvent)
        console.log( '===>>>> dateTime',dayjs(req.dateTime).format('YYYY-MM-DD HH:mm:ss'));
