...
export class UsersService {
    ...
    async create (data:UserDto):Promise<UserEntity> {
        try {
             const itemUser = await this.userRepasitory.create(data as unknown as UserEntity)
    ...
    async remove (id:string)  {
     ...
     async findByEmployeeNoString ({employeeNoString}:{employeeNoString:number}) {
        const itemUser = await this.userRepasitory.findOne({where:{employeeNoString}})
