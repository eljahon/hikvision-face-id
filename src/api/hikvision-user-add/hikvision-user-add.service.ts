...
const urlUserInfo = (api: string) => `http://${api}/ISAPI/AccessControl/UserInfo/Record?format=json`;
const urlUserInfoDelete = (api: string) => `http://${api}/ISAPI/AccessControl/UserInfo/Delete?format=json`;
const urlUserInfoUpdate = (api: string) => `http://${api}/ISAPI/AccessControl/UserInfo/Modify?format=json`;
@Injectable()
export class HikvisionUserAddService {
    ...
    constructor(private userService: UsersService, @InjectRepository(HikvisionUserAddEntity) private readonly hikvisionUserAddRepository: Repository<HikvisionUserAddEntity> ) {
    ...
    async userCreateInline(data: UserDto): Promise<UserEntity | boolean> {
        console.log(data, 'data create inline');
        const user = await this.userService.findByEmployeeNoString({employeeNoString: data.employeeNoString})
